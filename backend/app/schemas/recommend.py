...
class RecommendRequest(BaseModel):
    ...
    open_now: bool = Field(
        False, description="Whether to only include venues that are currently open"
    )
...
class RecommendMeta(BaseModel):
    ...
    time_taken_ms: int | None = Field(
        None, description="Time taken to generate the response in milliseconds"
    )
...
class VenueCard(BaseModel):
    ...
    explanations: list[str] | None = Field(
        None, description="Why this venue matches (2-3 bullet points)"
    )
